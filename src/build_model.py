...
def build_artifacts():
    ...
    # Add title features to prevent sequels from being mathematically identical
    movies['title_clean'] = movies['title'].str.replace(r'\(\d{4}\)', '', regex=True).str.strip()
    movies['combined_features'] = movies['title_clean'] + ' ' + movies['genres_clean']
    tfidf = TfidfVectorizer(stop_words='english', ngram_range=(1, 2))
    genre_matrix = tfidf.fit_transform(movies['combined_features'])
